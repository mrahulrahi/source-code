import React from 'react';
import './exam.css'
import InnerHeroBg from '../../assets/images/inner-img-11.jpg'
import ExamCardImg from '../../assets/images/exam-flag-icon.svg'
import ExamScheduleBg from '../../assets/images/exam-flag-icon.svg'

const exam = () => {
  return (
    <React.Fragment>

      <div className="innerherotwo" style={{ backgroundImage: `url(${InnerHeroBg})` }}>
        <div className="container">
          <div className="row">
            <div className="col-lg-12">
              <div className="ih-exam-card position-relative">
                <div className="ih-ec-img"><img src={ExamCardImg} alt="" /></div>
                <div className="ih-ec-content">
                  <h3>‘A school with mind, heart, and a constant fizz!’</h3>
                  <a href="#!" className="btn btn-default">See Results Here</a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="content-container exam-schedule-container" >
        <div className="exam-schedule-bg"><img src={ExamScheduleBg} alt="" /></div>
        <div className="container">
          <div className="row">
            <div className="col-xl-10 mx-auto">
              <h4>EXAM <span className="text-blue">SCHEDULE</span></h4>
              <div className="row g-4">
                <div className="col-sm-6 col-lg-4">
                  <div className="exam-schedule-col-box position-relative">
                    <div className="esc-img"><img src={ExamCardImg} alt="" /></div>
                    <div className="esc-content">
                      <div className="esc-top-label">GU-Theory Exam</div>
                      <h5>ODD 2022-23</h5>
                      <div className="exam-schedule-list">
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">DEC 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-sm-6 col-lg-4">
                  <div className="exam-schedule-col-box position-relative">
                    <div className="esc-img"><img src={ExamCardImg} alt="" /></div>
                    <div className="esc-content">
                      <div className="esc-top-label">GU-Theory Exam</div>
                      <h5>EVEN 2022-23</h5>
                      <div className="exam-schedule-list">
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Science (IT) - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-sm-6 col-lg-4">
                  <div className="exam-schedule-col-box position-relative">
                    <div className="esc-img"><img src={ExamCardImg} alt="" /></div>
                    <div className="esc-content">
                      <div className="esc-top-label">GU-Theory Exam</div>
                      <h5>EVEN 2022-23</h5>
                      <div className="exam-schedule-list">
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Science (IT) - Second-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                        <div className="exam-schedule-item">
                          <div className="exam-schedule-box d-flex align-items-center">
                            <div className="esb-date">JUN 23</div>
                            <div className="esb-content">Bachelor Of Commerce - First-Semester</div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

    </React.Fragment>
  )
}

export default exam